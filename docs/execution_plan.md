...
**Weekly Review (Every Sunday 18:00 CET)**:
1. Ingest last 504 trading days of price/volume data
...
3. **Skip if**: Drift < 15% OR TC > 25bps (cost too high)
4. Manual approval required before orders
...
Cash: iShares € Ultrashort Bond ETF (market order)

**Sizing Rules**:
- Max 25% daily volume per ETF
- Total turnover < 10% AUM per rebalance
- €1M portfolio → €100k max daily turnover
...
ETF prices/volumes (Yahoo Finance API)

VIX, 20-day spreads

Regime indicators (corr/vol)

Validation:
Price change < 20% daily